...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((key, value) in grades) {
        if (value in newGrades.keys) newGrades[value]?.add(key)
        newGrades.getOrPut(value) { mutableListOf(key) }
    }
    return newGrades
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    val countings = mutableMapOf<String, Int>()
    for (i in list) {
        if (i !in countings.keys) countings[i] = 1
        else countings[i] = countings[i]!! + 1
    }
    return countings.filter { it.value > 1 }
}
